...
Documents use a versioned envelope rather than serializing UI or kernel state. Imported STEP features embed validated source data and a stable shell entity id instead of a filesystem path or Truck object id, so they remain portable and rebuildable. Loading rejects unknown formats, future schema versions, duplicate feature IDs, dependency cycles, non-finite transforms, invalid dimensions, invalid colors, and invalid material metadata before replacing the active document. Every command is evaluated against a staged document and the active document is replaced only after the CAD kernel accepts the resulting graph and geometry.
...
2. STEP assemblies, units, colors, and manufacturing metadata workflows.
3. Assemblies, mates, and interference built on the persistent measurement foundation.
