...
CADX is an AI-native parametric 2D/3D CAD desktop application written in Rust 2024. It uses egui for the workbench, wgpu for the 3D viewport and 2D sketch overlays, Truck as the first CAD kernel, and rust-genai for provider-neutral model access.

CADX supports both 2D and 3D workflows as one connected model. The 2D side covers exact sketches, constraints, construction geometry, engineering annotations, projection/drawing checks, and ECAD layout data. The 3D side covers parametric solids, B-Rep topology, feature trees, assemblies, interference analysis, materials, and exchange formats. Sketches can drive 3D features, 3D faces can host sketches and datum references, and the MCAD/AEC/ECAD domain packs route the relevant 2D and 3D tools through the same document, transaction, undo/redo, and AI layers.
...
crates/
  ...
  cadx-domain-api/    Geometry-neutral DomainPack SPI, schema, solvers, shaders, AI tools
  cadx-mcad-*/        MCAD model, standards, DFM, BOM, and aggregate pack
